				...
				<form action="Articulos.php" class="form-group">
					...
					<div class="input-group">
						...
						<select name="proveedor_articulo" id="proveedor_articulo" class="form-control">
							...
							$consulta = mysqli_query($mysqli, $sql);
							while ($resultado = mysqli_fetch_array($consulta)) {
								echo '<option value=' . $resultado['id_proveedor'] . '>' . $resultado['id_proveedor'] . '</option>';
							}
							?>
						</select>
		...
		$('#articulos').click(function(e){
			...
			if (clave_a !='' && descripcion_a != '' && categoria_a !='' && invmin_a !='' && invmax_a !='' && compra_a !='' && utilidad_a !='' && cantidad_a !='' && proveedor_a !='') {
				...
				$.ajax({
					url: 'Crud.php',
					type: 'POST',
					data: {clave_a:clave_a, descripcion_a:descripcion_a, invmin_a:invmin_a, invmax_a:invmax_a, compra_a:compra_a, utilidad_a:utilidad_a, cantidad_a:cantidad_a, proveedor_a:proveedor_a, id:id, articulos:articulos},
					success: function (data) {
